...
export function activate(context: vscode.ExtensionContext) {
  ...
  const debugVariablesView = vscode.window.createTreeView(
    "mmbasic.debugVariables",
    {
  ...
  const commands = [
    ...
    vscode.commands.registerCommand("mmbasic.uploadFile", async () => {
      ...
      if (fileUri && fileUri[0]) {
        ...
        if (remoteName) {
          const success = await fileBrowser.uploadFile(
            fileUri[0].fsPath,
            remoteName,
          );
          if (success) {
            vscode.window.showInformationMessage(
              `File uploaded: ${remoteName}`,
            );
          }
        }
      }
    ...
    vscode.commands.registerCommand(
      "mmbasic.downloadFile",
      async (file: MMBasicFile) => {
